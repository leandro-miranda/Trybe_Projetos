...
const verifyFood = () => {
  let sumFood = 0;
  const { food } = restaurant.fetchMenu();
  const request = restaurant.consumption;
  request.forEach((item) => {
    if (typeof food[item] === 'number') {
      sumFood += food[item];
    }
  });
  return sumFood;
...
const verifyDrink = () => {
  let sumDrink = 0;
  const { drink } = restaurant.fetchMenu();
  const request = restaurant.consumption;
  request.forEach((item) => {
    if (typeof drink[item] === 'number') {
      sumDrink += drink[item];
    }
  });
  return sumDrink;
...
const createMenu = (parameter) => {
  restaurant.fetchMenu = () => parameter;
  restaurant.consumption = [];
  restaurant.order = orderFromMenu;
  restaurant.pay = totalPay;
  return restaurant;
};
