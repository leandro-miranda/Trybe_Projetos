...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(typeof radius).toEqual('object');
    // Teste se o objeto retornado possui 3 propriedades.
    const properties = Object.keys(radius);
    ...
    const radiusTwo = circle(2);
    const circumferenceValue = radiusTwo.circumference;
    expect(circumferenceValue).toEqual(12.56);
    // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.
    const radiusThree = circle(3);
    expect(radiusThree.radius).toEqual(3);
    const areaValue = (radiusThree.area);
    expect(areaValue).toEqual(28.259999999999998);
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    const circumferenceValueThree = radiusThree.circumference;
    expect(circumferenceValueThree).toEqual(18.84);
  });
});
